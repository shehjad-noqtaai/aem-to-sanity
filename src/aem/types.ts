...
/**
 * Granite UI dialog nodes in JSON form (what `.model.json` returns when the
 ...
 * by child node name.
 *
 * We keep this lenient: AEM exposes many attributes we don't care about; `.catchall()`
 * preserves them so container nodes can still be walked without losing children.
 */
...
/**
 * Returns the direct child dialog nodes of a given node, in their declared
 * order. A "child node" is any property whose value is a plain object (i.e. not
 * a primitive attribute like `jcr:title`). Attributes beginning with `jcr:` or
 * `sling:` that happen to be objects are excluded defensively.
 */
...
export function isTruthyAttr(v: unknown): boolean {
  ...
  if (typeof v === "string") return v.toLowerCase() === "true" || v === "{Boolean}true";
  return false;
}
