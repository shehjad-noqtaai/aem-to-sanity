import { defineField, defineType } from "sanity";

/**
 * Generated from AEM component: /apps/aem-integration/components/faq-hub
 * DO NOT EDIT BY HAND — regenerate via `pnpm migrate:schema`.
 */
export const faqHub = defineType({
  name: "faqHub",
  title: "FAQ Hub",
  type: "object",
  groups: [{ name: "properties", title: "Properties" }],
  preview: {
    select: {
      prMedia: "sections.0.fileReference",
    },
    prepare({ prMedia }) {
      return {
        title: "FAQ Hub",
        media: prMedia,
      };
    },
  },
  fields: [
    defineField({
      name: "headline1",
      title: "Headline 1 (script)",
      type: "string",
    }),
    defineField({
      name: "headline2",
      title: "Headline 2 (sans serif)",
      type: "string",
    }),
    defineField({
      name: "description",
      title: "Description",
      type: "array",
      of: [{ type: "block" }],
    }),
    defineField({
      name: "removeTopPadding",
      type: "boolean",
      initialValue: true,
    }),
    defineField({
      name: "removeBottomPadding",
      type: "boolean",
      initialValue: true,
    }),
    defineField({
      name: "sections",
      title: "FAQ Sections",
      type: "array",
      group: "properties",
      of: [
        {
          type: "object",
          title: "FAQ Sections",
          fields: [
            defineField({
              name: "sectionTitle",
              title: "Section title",
              type: "string",
              validation: (Rule) => Rule.required(),
            }),
            defineField({
              name: "fileReferenceAemPath",
              title: "Category image (AEM DAM path)",
              description:
                "Original AEM path from migration (read-only). Use the Sanity asset field below for previews and delivery.",
              type: "string",
              readOnly: true,
            }),
            defineField({
              name: "fileReference",
              title: "Category image",
              type: "image",
              validation: (Rule) => Rule.required(),
            }),
            defineField({
              name: "nestedLinks",
              title: "Section links",
              type: "array",
              of: [
                {
                  type: "object",
                  title: "Section links",
                  fields: [
                    defineField({
                      name: "text",
                      title: "Link text",
                      type: "string",
                      validation: (Rule) => Rule.required(),
                    }),
                    defineField({
                      name: "link",
                      title: "Link location",
                      type: "string",
                      validation: (Rule) => Rule.required(),
                    }),
                  ],
                },
              ],
            }),
          ],
        },
      ],
    }),
    defineField({
      name: "buttons",
      title: "Buttons",
      type: "array",
      of: [
        {
          type: "object",
          title: "Buttons",
          fields: [
            defineField({
              name: "text",
              title: "Button text",
              type: "string",
              validation: (Rule) => Rule.required(),
            }),
            defineField({
              name: "link",
              title: "Button link",
              type: "string",
              validation: (Rule) => Rule.required(),
            }),
            defineField({
              name: "buttonHexColor",
              title: "Button color",
              type: "string",
            }),
            defineField({
              name: "type",
              title: "Button type",
              type: "string",
              options: {
                list: [
                  { title: "Ghost", value: "ghost" },
                  { title: "Filled", value: "filled" },
                  { title: "link", value: "link" },
                ],
              },
            }),
            defineField({
              name: "ctaTextHexColor",
              title: "CTA Text Color",
              type: "string",
            }),
          ],
        },
      ],
    }),
  ],
});
